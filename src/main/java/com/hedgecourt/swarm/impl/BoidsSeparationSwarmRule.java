...
public class BoidsSeparationSwarmRule extends AbstractSwarmRule implements SwarmRule {

	public BoidsSeparationSwarmRule() {
		super();
	}
	...
	public DesiredVelocity getDesiredVelocity(Speck s, List<Speck> neighbors) {
		...
		if (s.type() != Speck.TYPE_PREY || neighbors.size() == 1) {
			...
			if (aVels.size() > 0) {
				...
				dVel.setWeight(SwarmConfig.RULE_WEIGHT_SEPARATION);
			} else {
